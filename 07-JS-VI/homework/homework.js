...
function sumarArray(numeros, cb) {
  ...
  //Tu código:
  let suma = numeros.reduce((acc, element)=>{return acc + element})
  cb(suma)
}
...
function map(array, cb) {
  ...
  //Tu código:
  let newArray = array.map((elemento)=>{return cb(elemento)})
  return newArray
}
...
function filter(array) {
  ...
  //Tu código:
  let arrayGado = [];
  arrayGado = array.filter(cumplen => cumplen.charAt(0) == "a");
  return arrayGado;
}
